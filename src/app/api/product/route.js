  ...
  try {
    ...
    const formattedProducts = await Promise.all(
      products.map(async (product) => {
        ...
        if (!categoryData) isAvailable = 0;

        // VARIANTS CHECK
        const variants = await productVariantSchema.find({
          productId: product._id,
        });
        if (!variants.length) isAvailable = 0;
        console.log("variant======", variants);

        // COLOR + SIZE TABLE CHECK
        for (const variant of variants) {
          const colorExists = await colorSchema.findOne({
            _id: variant.color,
            status: 1,
          });
          if (!colorExists) {
          ...
          const sizeExists = await sizeSchema.findOne({
            _id: variant.size,
            status: 1,
          });
          if (!sizeExists) {
        ...
        // AUTO UPDATE PRODUCT isAvailable
    ...
    const stock = await productVariantSchema.collection
      .aggregate([
        ...
        { $match: { "product.isDeleted": 0 } },
        {
          $group: {
            _id: "$productId",
            hasStock: { $max: { $cond: [{ $gt: ["$stock", 0] }, 1, 0] } },
          },
        },
        {
          $group: {
            _id: null,
            inStock: { $sum: { $cond: [{ $eq: ["$hasStock", 1] }, 1, 0] } },
            outStock: { $sum: { $cond: [{ $eq: ["$hasStock", 0] }, 1, 0] } },
          },
        },
        { $project: { _id: 0, inStock: 1, outStock: 1 } },
      ])
      .toArray();
